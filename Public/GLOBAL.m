...
%--------------------------------------------------------------------------
    ...
    properties(SetAccess = ?PROBLEM)
        ...
        CurrentColumn;                  %Numero de elementos diferentes en cada columna
        CorrelationMeasure;             %Medida de corrrelacion
        ArrElemDiff;                    %Arreglo que contiene todos los numeros diferentes
        ArrY;                           %Arreglo que contiene los resultados correctos de cada iteracion
        ArrX;                           % Arreglo que contiene la columna que se esta optimizando
    ...
    methods
        ...
        %              '-M',2,'-D',10)
            ...
            propertyStr = {'N','M','D','algorithm','problem','evaluation','run','save','outputFcn','CurrentColumn','CorrelationMeasure','ArrElemDiff','ArrY','ArrX'};
            if nargin > 0
        ...
        function Output(obj)
            ...
            if obj.evaluated >= obj.evaluation
                if obj.save == 0
                    ...
                    folder = fullfile('@MODiscretizer',func2str(obj.algorithm));
                    [~,~]  = mkdir(folder);
                    result         = obj.result;
                    metric.runtime = obj.runtime;
                    save(fullfile(folder,'Output'));
                end
            end
        end
